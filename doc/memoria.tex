...
retrain_3fc.py: script basado en retrain.py, en vez de añadir una sola capa añade 3.


Prueba quitar capas

retrain_3fc.py: script basado en retrain.py, acepta una opción para especificar la capa desde la que se quiere entrenar.


Prueba añadir una capa sets bien repartidos

acdc_test.sh: llama a los scripts retrain.py, batch_inference.py y per_patient_prediction.py.

retrain.py: aplica la técnica de transfer learning en el dataset ACDC.

batch_inference.py: ejecuta la inferencia en todas las imágenes de un conjunto.


Prueba usar un slice por paciente

acdc_one_slice_test.sh: llama a los scripts acdc_one_slice_retrain.py y acdc_one_slice_batch_inference.py.

acdc_one_slice_retrain.py: aplica la técnica de transfer learning en el dataset ACDC usando solo un slice por paciente.

acdc_one_slice_batch_inference.py: ejecuta la inferencia en todas las imágenes de un conjunto.


Prueba entrenamiento por subsets

acdc_subset_test.sh: llama a los scripts acdc_subset_retrain.py y acdc_subset_batch_inference.py una vez por cada clase del dataset ACDC.

acdc_subset_retrain.py: aplica la técnica de transfer learning en el dataset ACDC, acepta un parámetro para dividir las clases del dataset ACDC en subconjuntos.

acdc_subset_batch_inference.py: ejecuta la inferencia en todas las imágenes de un conjunto.


Prueba cascada

acdc_cascade_test.sh: aplica la técnica de cascada haciendo uso del script acdc_subset_retrain.py y llama al script acdc_cascade_batch_inference.py.

acdc_cascade_batch_inference.py: ejecuta la inferencia en todas las imágenes de un conjunto, usando la técnica de cascada.


Prueba inferencia por paciente

per_patient_prediction.py: usa los csv generados por los scripts de inferencia para dar un resultado por paciente.


Prueba distorsión

acdc_distortion.sh: aplica rotacion de 90, 180 y 270 grados a todas las imágenes del dataset ACDC.


Otras pruebas

flowers_test.sh: llama a los scripts retrain.py y batch_inference.py, con el dataset flowers.


Otros scripts

run.sh: script principal que ejecuta todo, prepara los datasets, llama a los scripts para reunir la información y llama al script test_all.sh, que ejecuta todas las pruebas.

acdc_extract.sh: descomprime el zip del dataset ACDC y llama al script acdc_nii_to_jpg.py.

acdc_nii_to_jpg.py: convierte las imágenes del dataset ACDC que están en formato nii a formato jpg.
...
test_all.sh: ejecuta todas las pruebas, el script de distorsión, vuelve a ejecutar todas las pruebas y llama al script summary.sh.
