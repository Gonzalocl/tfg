...
El código de este trabajo se puede encontrar en aquí: https://drive.google.com/drive/folders/1q71pmSSp85m0L4aWFQi78J12YuodHoXS?usp=sharing.
...
Prueba añadir una capa totalmente conectada y softmax

retrain.py: aplica la técnica de transfer learning.


Prueba añadir 3 capas totalmente conectadas

retrain\_3fc.py: script basado en retrain.py, en vez de añadir una sola capa añade 3.


Prueba quitar capas

retrain\_3fc.py: script basado en retrain.py, acepta una opción para especificar la capa desde la que se quiere entrenar.


Prueba añadir una capa sets bien repartidos

acdc\_test.sh: llama a los scripts retrain.py, batch\_inference.py y per\_patient\_prediction.py.

retrain.py: aplica la técnica de transfer learning en el dataset ACDC.

batch\_inference.py: ejecuta la inferencia en todas las imágenes de un conjunto.


Prueba usar un slice por paciente

acdc\_one\_slice\_test.sh: llama a los scripts acdc\_one\_slice\_retrain.py y acdc\_one\_slice\_batch\_inference.py.

acdc\_one\_slice\_retrain.py: aplica la técnica de transfer learning en el dataset ACDC usando solo un slice por paciente.

acdc\_one\_slice\_batch\_inference.py: ejecuta la inferencia en todas las imágenes de un conjunto.


Prueba entrenamiento por subsets

acdc\_subset\_test.sh: llama a los scripts acdc\_subset\_retrain.py y acdc\_subset\_batch\_inference.py una vez por cada clase del dataset ACDC.

acdc\_subset\_retrain.py: aplica la técnica de transfer learning en el dataset ACDC, acepta un parámetro para dividir las clases del dataset ACDC en subconjuntos.

acdc\_subset\_batch\_inference.py: ejecuta la inferencia en todas las imágenes de un conjunto.


Prueba cascada

acdc\_cascade\_test.sh: aplica la técnica de cascada haciendo uso del script acdc\_subset\_retrain.py y llama al script acdc\_cascade\_batch\_inference.py.

acdc\_cascade\_batch\_inference.py: ejecuta la inferencia en todas las imágenes de un conjunto, usando la técnica de cascada.


Prueba inferencia por paciente

per\_patient\_prediction.py: usa los csv generados por los scripts de inferencia para dar un resultado por paciente.


Prueba distorsión

acdc\_distortion.sh: aplica rotacion de 90, 180 y 270 grados a todas las imágenes del dataset ACDC.


Otras pruebas

flowers\_test.sh: llama a los scripts retrain.py y batch\_inference.py, con el dataset flowers.


Otros scripts

run.sh: script principal que ejecuta todo, prepara los datasets, llama a los scripts para reunir la información y llama al script test\_all.sh, que ejecuta todas las pruebas.

acdc\_extract.sh: descomprime el zip del dataset ACDC y llama al script acdc\_nii\_to\_jpg.py.

acdc\_nii\_to\_jpg.py: convierte las imágenes del dataset ACDC que están en formato nii a formato jpg.

cines.sh: a partir de las imagenes extraidas del dataset ACDC, las convierte en videos para poder visualizar el dataset.

info.sh: genera un csv con la información que se proporciona de cada paciente del dataset ACDC.

test\_all.sh: ejecuta todas las pruebas, el script de distorsión, vuelve a ejecutar todas las pruebas y llama al script summary.sh.

summary.sh: genera un csv donde reúne los resultados de las pruebas ejecutadas.
